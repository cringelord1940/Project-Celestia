...
export class GLTFAnimation {
  constructor(data, weight = 1) {
    ...
    this.duration = data.reduce(
      (a, { times }) => Math.max(a, times[times.length - 1]),
      0
    )
  }
  ...
  update(totalWeight = 1, isSet) {
    ...
    this.data.forEach(({ node, transform, interpolation, times, values }) => {
      ...
      let alpha =
        (elapsed - times[prevIndex]) / (times[nextIndex] - times[prevIndex])
      ...
      if (interpolation === 'CUBICSPLINE') {
        ...
        prevVal = this.cubicSplineInterpolate(
          alpha,
          prevVal,
          prevTan,
          nextTan,
