...
export class GLTFLoader {
      ...
      desc.meshes.forEach(({ primitives }) => {
        primitives.forEach(({ attributes, indices }) => {
          ...
          for (let attr in attributes)
            bufferViews[
              desc.accessors[attributes[attr]].bufferView
            ].isAttribute = true
          ...
          bufferViews[desc.accessors[indices].bufferView].target =
  ...
  static parseImages(gl, desc, dir, bufferViews) {
    ...
    return desc.images.map(
      ({ uri, bufferView: bufferViewIndex, mimeType, name }) => {
  ...
  static parseTextures(gl, desc, images) {
    ...
    return desc.textures.map(
      ({
        ...
        const sampler =
          samplerIndex !== undefined ? desc.samplers[samplerIndex] : null
        if (sampler) {
  ...
  static parseSkins(gl, desc, bufferViews) {
    ...
    return desc.skins.map(
      ({
        ...
        return {
          inverseBindMatrices: this.parseAccessor(
            inverseBindMatrices,
            desc,
            bufferViews
          ),
    ...
    return primitives.map(
      ({
        ...
        if (indices !== undefined) {
          geometry.addAttribute(
            'index',
            this.parseAccessor(indices, desc, bufferViews)
          )
        }
      ...
      ({
        ...
        const data = channels.map(
          ({
            ...
            const values = this.parseAccessor(
              outputIndex,
              desc,
              bufferViews
            ).data
