...
float domainWarp(vec3 uv) {
  ...
  for(float i = 0.; i < 10.; i += 1.) {
    vec3 offset = vec3((i + uTime) * 0.1, (i + uTime) * 0.2, (i + uTime) * 0.3);

    vec3 dUv = vec3(gln_simplex_fbm((scale * uv) + outUv, opts), gln_simplex_fbm((scale * uv) + outUv, opts), gln_simplex_fbm((scale * uv) + outUv, opts));
...
vec3 saturateColor(vec3 v, float s) {
  return vec3(clamp(v.x, 0., s), clamp(v.y, 0., s), clamp(v.z, 0., s));
}
