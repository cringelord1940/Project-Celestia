...
const BlockComponents: BlockComponentsType = {
  paragraph: ({ content }) => (
    <p className='py-2 text-[#5c5c5c] dark:text-[#bbbbbb]'>
      &emsp;&emsp;&emsp;
      <Text content={content.children as TextContent[]} />
    </p>
  ),
  'heading-one': ({ content }) => (
  ...
  class: ({ content }) => (
    <div className={clsx(content.className)}>
      <ContentFromRaw raw={content.children as any} />
    </div>
  ),
  'block-quote': ({ content }) => (
    <blockquote
...
const ContentFromRaw = ({ raw }: { raw: Content[] }) => {
  return (
    <>
      {raw.map((content, index) => {
        const Component = BlockComponents[content.type as POST.CONTENT_BLOCK]
