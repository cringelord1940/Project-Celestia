...
function Cursor() {
  ...
  const onMouseMove2 = useCallback((event: PointerEvent) => {
    event.preventDefault()
    const { clientX, clientY, currentTarget } = event
    const { x: rectX, y: rectY } = currentTarget.getBoundingClientRect()
    ...
    if (cursor2.current) {
      cursor2.current.style.transform = `translate3d(${x}px, ${y}px, 0px)`
    }
  }, [])
